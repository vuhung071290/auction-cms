...
const auction = (state = initialState, action) => {
    switch (action.type) {
        case Action.AUCTION_LIST_INIT:
            const { list, total, page, size } = action;
            return { ...state, list, total, page, size };
        case Action.AUCTION_LIST_LOADING:
            return { ...state, loading: action.status };
        case Action.AUCTION_SEARCH_CHANGE:
            return { ...state, search: action.search };
        case Action.AUCTION_DATE_CHANGE:
            return { ...state, startDateSearch: action.startDateSearch, endDateSearch: action.endDateSearch };
        case Action.AUCTION_PAGE_CHANGE:
            return { ...state, page: action.page };
        case Action.AUCTION_SIZE_CHANGE:
